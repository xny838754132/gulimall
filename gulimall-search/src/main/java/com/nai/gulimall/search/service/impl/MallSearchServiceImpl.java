package com.nai.gulimall.search.service.impl;
...
import com.nai.gulimall.search.config.GuliMallElasticSearchConfig;
import com.nai.gulimall.search.constant.EsConstant;
import com.nai.gulimall.search.feign.ProductFeignService;
import com.nai.gulimall.search.service.MallSearchService;
import com.nai.gulimall.search.vo.AttrResponseVo;
import com.nai.gulimall.search.vo.BrandVo;
...
public class MallSearchServiceImpl implements MallSearchService {
  ...
  private SearchRequest buildSearchRequest(SearchParam param) {
    ...
    // 品牌聚合的子聚合
    brand_agg.subAggregation(
        AggregationBuilders.terms("brand_name_agg").field("brandName").size(1));
  ...
  private SearchResult buildSearchResult(SearchResponse response, SearchParam param) {
    ...
    for (Terms.Bucket bucket : brand_agg.getBuckets()) {
      ...
      // 3、得到品牌的图片
      String brandImg =
          ((ParsedStringTerms) bucket.getAggregations().get("brand_img_agg"))
              .getBuckets()
              .get(0)
              .getKeyAsString();
    ...
    int totalPages =
        (int) total % EsConstant.PRODUCT_PAGE_SIZE == 0
          ...
          param.getAttrs().stream()
              .map(
                  attr -> {
                    ...
                    if (r.getCode() == 0) {
                      AttrResponseVo data =
                          r.getData("attr", new TypeReference<AttrResponseVo>() {});
