...
public class LoginController {
    ...
    /**
     * The Member feign service.
     */
    @Autowired
    MemberFeignService memberFeignService;

    /**
     * 发送一个请求直接跳转到一个页面 SpringMvc viewController:将请求和页面映射过来 @param phone the phone
     *
     * @return the r
     */
    @ResponseBody
    ...
    public R sendCode(@RequestParam("phone") String phone) {
        ...
        if (StringUtils.isNotEmpty(redisCode)) {
            ...
            if (System.currentTimeMillis() - time < 60000) {
                // 60S内不能再发
                return R.error(
                        BizCodeEnum.VALID_CODE_EXCEPTION.getCode(),
                        BizCodeEnum.VALID_CODE_EXCEPTION.getMessage());
            }
        }
        ...
        redisTemplate
                .opsForValue()
                .set(AuthServerConstant.SMS_CODE_CACHE_PREFIX + phone, subString, 10,
                        TimeUnit.MINUTES);
    ...
    /**
     * TODO 重定向携带数据,利用session原理.将数据存放在session中,只要跳到下一个页面,取出数据以后,session里面的数据就会删掉 TODO
     * 1.分布式下的session问题. RedirectAttributes redirectAttributes: 模拟重定向携带数据
     *
     * @param registeredVo       the registered vo
    ...
    public String registered(
            @Valid UserRegisteredVo registeredVo,
        ...
        String redidCode =
                redisTemplate
                        .opsForValue()
                        .get(AuthServerConstant.SMS_CODE_CACHE_PREFIX + registeredVo.getPhone());
        if (StringUtils.isNotEmpty(redidCode)) {
            if (code.equals(redidCode.split("_")[0])) {
                // 删除验证码;令牌机制
                redisTemplate
                        .delete(AuthServerConstant.SMS_CODE_CACHE_PREFIX + registeredVo.getPhone());
                // 验证码通过.
                // 真正注册 调用远程进行注册
    ...
    @PostMapping("/login")
    public String login(
            UserLoginVo loginVo, RedirectAttributes redirectAttributes, HttpSession session) {
